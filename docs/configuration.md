...
For the `jitsi` profile, `meet.${DOMAIN_NAME}` stays on Pangolin for web and signaling while `rtc.${DOMAIN_NAME}` points directly at your home server for JVB and TURN media. The full DNS-only Cloudflare record setup and home router port-forward steps live in [Jitsi Meet](jitsi.md).
