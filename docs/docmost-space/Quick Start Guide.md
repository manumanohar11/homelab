...
| **Grafana** | `http://your-server:3000` | Sign in with `GRAFANA_ADMIN_USER` and the password from `.env` |
...
See [Configuration Guide](Configuration Guide.md) for the recommended board layout.

### Configure Docmost

1. Open `http://your-server:3004`
2. Create the first workspace admin and a space such as `Homelab Operations`
3. Build the import bundle with `python3 scripts/build-docmost-space.py`
4. Import `docs/docmost-space/` into Docmost
5. Keep editing the canonical repo docs, then rebuild and re-import when you want the wiki copy refreshed
...
See [Backup Guide](Backup & Recovery Guide.md) for detailed setup.

### Set Up Monitoring

1. Open Grafana: `http://your-server:3000`
2. Login with `GRAFANA_ADMIN_USER` and the password from `.env`
3. Change the password if you used a temporary value during setup
4. Explore the pre-configured dashboards and datasources
...
See [Configuration Guide](Configuration Guide.md#vpn-setup) for VPN setup.
