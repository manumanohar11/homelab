...
Operator catalog for the Media Stack.

This page prioritizes user-facing services and the critical backends you may need to troubleshoot. For the full rendered inventory, use `docker compose config --services`.
...
Exact image tags and digests live in the compose files. The image column here is intentionally kept at the repository-image level to stay readable.
...
| **Gluetun** | varies | VPN gateway that publishes the routed app ports | `qmcgaw/gluetun` | ✅ |
| **Newt** | - | Pangolin secure tunnel | `fosrl/newt` | ✅ |
...
Private, app-friendly calling services with Pangolin for web entry and home-hosted media.

| Service | Port | Description | Image | Status |
|:--------|:----:|:------------|:------|:------:|
| **Jitsi Meet** | 443 + home media ports | Web/app entrypoint for private video calling | `jitsi/*`, `coturn/coturn`, `python:3.12-alpine` | 📦 `jitsi` |

### Jitsi Meet

- Enable with `docker compose --profile jitsi up -d`
- Public URL: `https://meet.${DOMAIN_NAME}`
- Home media hostname: `rtc.${DOMAIN_NAME}`
- Pangolin SSO must stay disabled on the `meet` HTTP resource so the Jitsi mobile app can connect cleanly
- Home router must forward `10000/udp`, `3478/udp`, `3478/tcp`, and `20000-20100/udp` to the Docker host
- `rtc.${DOMAIN_NAME}` must stay DNS-only in Cloudflare so media never relays through the VPS
- Do not create Pangolin raw resources, Gerbil mappings, or Traefik UDP/TCP entrypoints for Jitsi
- After the stack is up, create one moderator account with `docker compose exec jitsi-prosody prosodyctl --config /config/prosody.cfg.lua register "${JITSI_AUTH_USER}" "${JITSI_XMPP_DOMAIN}" "${JITSI_AUTH_PASSWORD}"`
- Full deployment and troubleshooting details live in [Jitsi Meet](Jitsi Meet.md)
...
| Service | Port | Description | Image | Status |
|:--------|:----:|:------------|:------|:------:|
| **Plex** | 32400 | Premium media server with GPU transcoding | `lscr.io/linuxserver/plex` | ✅ |
...
| **Stash** | 9998 | Adult media organizer | `stashapp/stash` | 📦 `stash` |
...
| **PostgreSQL** | 5432 | Immich database | `ghcr.io/immich-app/postgres` | ✅ |
| **Redis** | 6379 | Immich cache | `docker.io/valkey/valkey` | ✅ |
...
| Service | Port | Description | Image | Status |
|:--------|:----:|:------------|:------|:------:|
| **Radarr** | 7878 | Movie collection manager | `lscr.io/linuxserver/radarr` | 📦 `arr` |
| **Sonarr** | 8989 | TV show collection manager | `lscr.io/linuxserver/sonarr` | 📦 `arr` |
| **Lidarr** | 8686 | Music collection manager | `lscr.io/linuxserver/lidarr` | 📦 `arr` |
| **Readarr** | 8787 | Book collection manager | `lscr.io/linuxserver/readarr` | 📦 `arr` |
| **Bazarr** | 6767 | Subtitle manager | `lscr.io/linuxserver/bazarr` | 📦 `arr` |
...
| Service | Port | Description | Image | Status |
|:--------|:----:|:------------|:------|:------:|
| **qBittorrent** | 8080 | Torrent client | `lscr.io/linuxserver/qbittorrent` | 📦 `arr` / `downloaders` |
| **Prowlarr** | 9696 | Indexer manager | `lscr.io/linuxserver/prowlarr` | 📦 `arr` / `downloaders` |
...
**Login:** `GRAFANA_ADMIN_USER` and the password from `.env`
...
Paperless also includes Gotenberg and Tika sidecars. Karakeep includes Meilisearch and a headless Chrome sidecar. Use `docker compose config --services` when you need the full backend inventory.
...
The repo also ships a simplified import-ready wiki bundle under `docs/docmost-space/`.
...
- covers the Immich, Joplin, Paperless, and Docmost PostgreSQL containers
...
- [Jitsi Meet](Jitsi Meet.md) - Home-media setup, moderator flow, and media troubleshooting
- [Networking](Networking Guide.md) - Port mappings and VPN routing
