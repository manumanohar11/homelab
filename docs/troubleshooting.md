...
<details>
<summary><strong>Docmost won't start</strong></summary>

**Check the application and dependencies:**
```bash
docker compose logs docmost --tail=100
docker compose logs docmost-db --tail=100
docker compose logs docmost-redis --tail=100
docker inspect --format='{{.State.Health.Status}}' docmost
```

**Common causes:**
- `DOCMOST_APP_SECRET` or `DOCMOST_DB_PASSWORD` is missing in `.env`
- `docmost-db` is unhealthy or its data directory is not writable
- `DOCMOST_BASE_URL` does not match the real public URL

</details>

<details>
<summary><strong>Docmost loads but collaboration/editor features are flaky</strong></summary>

If Docmost is behind a custom reverse proxy, confirm WebSocket upgrades are allowed and the public URL matches `DOCMOST_BASE_URL`.

```bash
docker compose logs docmost --tail=100 | grep -Ei 'ws|websocket|error'
curl -f http://localhost:3004/api/health
```

If you only see issues when accessing it remotely, compare local access at `http://your-server:3004` with the tunneled or proxied URL.
