...
if [[ "$MODE" == "--check" ]]; then
    if [[ $drift -ne 0 ]]; then
        echo "logging stack runtime config is out of sync with config-templates/" >&2
        exit 1
    fi
    echo "logging stack runtime config matches config-templates/"
else
    echo "logging stack runtime config synced from config-templates/"
fi
